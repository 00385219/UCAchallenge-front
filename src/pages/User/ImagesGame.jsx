...
import '../../assets/css/ImagesGame.css'; // Asegúrate de tener el CSS adecuado
...
function ImagesGame() {
    ...
    useEffect(() => {
        const fetchPreguntas = async () => {
            try {
                ...
                setPreguntas(preguntasConImagenes);
                setPreguntaActual(elegirPreguntaAleatoria(preguntasConImagenes));
            } catch (error) {
    ...
    const elegirPreguntaAleatoria = (preguntas) => {
        ...
        const preguntasDeTipo = preguntas.flatMap(quiz => quiz.questions.filter(question => question.type === "imagenes"));
        ...
        <div className="game-header">
            ...
            <div className="options-container">
                {preguntaActual && preguntaActual.options.map((option, index) => (
                    <label key={index} className="option-label image-option">
                        <input
                            type="radio"
                            name="imageChoice"
                            value={option._id} // Aquí asumimos que cada opción tiene un ID único
                            checked={selectedOption === option._id}
                            onChange={handleOptionChange}
                            className="option-input"
                        />
                        <img src={option.optionText} />
                    </label>
                ))}
            </div>
